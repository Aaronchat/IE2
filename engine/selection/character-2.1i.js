...
import * as random from "./random/character-2.1i.js";
...
const VALUES = Object.freeze({
  ...
  pregnancy: CHARACTER_PHYSICAL_APPEARANCE.pregnancy,
...
const MANUAL_ONLY = new Set(["hair-secondary-color", "hair-color-treatment", "hair-length", "pregnancy"]);
...
export function selectCharacter(controls = {}, context) {
  ...
  if (controls["skin-condition"]) {
    assertMode(controls["skin-condition"], ["manual", "random"], "Character skin condition");
    if (controls["skin-condition"].mode === "random") {
      out["skin-condition"] = result("random", Object.freeze([random.selectRandomSkinCondition(context)]));
    } else {
  ...
  if (controls.features) {
    assertMode(controls.features, ["manual", "random"], "Character features");
    if (controls.features.mode === "random") {
      out.features = result("random", Object.freeze([random.selectRandomFeature(context)]));
    } else {
